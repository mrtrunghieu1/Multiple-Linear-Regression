...
class MLR(object):
    ...
    def weight_caculation(self):
        ...
        for m in range(self.n_classes):
            X = self.P[:, m:(m + (self.n_classifiers - 1) *
                             self.n_classes) + 1: self.n_classes]
            y = Y[:, m]
            W[:, m] = lsq_linear(X, y).x
        train_model = []
        for i in range(self.n_classifiers):
            model = Posterior.PosteriorModel(
                self.features_X_train_full, self.labels_y_train_full, self.flag[i])
        ...
        P_temp = Posterior.PosteriorTest(train_model[0], meas_test)
        for i in range(1, self.n_classifiers):
            Pr_test = Posterior.PosteriorTest(train_model[i], meas_test)
